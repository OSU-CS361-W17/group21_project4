...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        get("/model", (req, res) -> newModel("hard"));
        //This will listen to POST requests and expects to receive a game model, as well as location to fire to
        post("/hard", (req, res) -> newModel("hard"));
        post("/easy", (req, res) -> newModel("easy"));
    ...
    private static String newModel(String diff) {
        if (diff.equalsIgnoreCase("hard")){
            BattleshipModel bm = new Hard();
            Gson gson = new Gson();
            return gson.toJson(bm);
        } else {
            BattleshipModel bm = new Easy();
            Gson gson = new Gson();
            return gson.toJson(bm);
        }
    }
